...
In the instructions it tells us, that this method should return an empty hash.
...
To pass this error message, we need to add a key `:pieces` to our empty hash, which is the value of our `railroads` and set its value to `4`. 
...
```
One of this methods tests are passing now. The next error message is: 
...
```
Here, we will set the third key of our railroads hash to `rent_in_dollars` with a empty hash as its value.
...
```
The first thing we need to do for this method is, to add a key `:one_piece_owned` with a `25` value to the empty hash we created as a value for `:rent_in_dollars` in the previous method.
...
To pass all of this error messages, we need to add a key/value pair to all our empty value hashes in our code.
